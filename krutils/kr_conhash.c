...
static T_KRActualNode *kr_conhash_create_node(char *id, unsigned int weights, void *priv);
static void kr_conhash_free_node(T_KRActualNode *node, T_KRConHash *krch);
...
static T_KRActualNode *kr_conhash_create_node(char *id, unsigned int weights, void *priv)
{
    T_KRActualNode *node = kr_malloc(sizeof(T_KRActualNode));
    if (node == NULL) {
    ...
    node->kicks = 0;
    node->vnodes = kr_calloc(weights * sizeof(T_KRVirtualNode));
    if (node->vnodes == NULL) {
        fprintf(stderr, "kr_malloc node->vnodes failed!\n");
        return NULL;
    }
    node->priv = priv;
...
{
    T_KRConHash *krch = kr_malloc(sizeof(T_KRConHash));
    if (krch == NULL) {
        fprintf(stderr, "kr_malloc krch failed!\n");
        return NULL;
    }
    krch->free_func = free_func;
...
{
    kr_skiplist_foreach(krch->node_list, \
        (KRForEachFunc )kr_conhash_free_node, krch);
...
int kr_conhash_add(T_KRConHash *krch, char *id, unsigned int weights, void *priv)
{
    ...
    node = kr_conhash_create_node(id, weights, priv);
    if (node == NULL) {
...
int kr_conhash_remove(T_KRConHash *krch, char *id)
{
    ...
    kr_conhash_free_node(node, krch);
    return 0;
}
...
{
    T_KRActualNode *del_node = NULL;
    del_node = kr_conhash_lookup(krch, id);
    if (del_node == NULL) {
        fprintf(stderr, "id[%s] not found\n", id);
        return -1;
    }
    
    T_KRActualNode *add_node = NULL;
    add_node = kr_conhash_create_node(id, weights, del_node->priv);
    if (add_node == NULL) {
        fprintf(stderr, "kr_conhash_create_node failed\n");
        return -1;
    }
    
    kr_conhash_delete_node(krch, del_node);
    kr_conhash_free_node(del_node, krch);
    
    kr_conhash_insert_node(krch, add_node);
...
{
    kr_skiplist_foreach(krch->node_list, foreach_func, data);
}
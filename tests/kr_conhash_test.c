...
int main(int argc, char *argv[])
{
    ...
    krch = kr_conhash_construct(NULL, NULL);
    assert(krch);
    assert(krch->free_func == NULL);
    result = kr_conhash_add(krch, machines[0], 9, NULL);
    assert(result == 0);
    result = kr_conhash_add(krch, machines[1], 3, NULL);
    assert(result == 0);
    result = kr_conhash_add(krch, machines[2], 12, NULL);
    assert(result == 0);
    result = kr_conhash_add(krch, machines[3], 4, NULL);
    assert(result == 0);
    result = kr_conhash_add(krch, machines[4], 8, NULL);
    assert(result == 0);

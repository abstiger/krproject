...
T_KRHDI *kr_hdi_construct(T_KRShmHDIDef *hdi_def)
{
    ...
    if (hdi_def->caHdiType[0] == KR_RETRIEVE_DATASRC) {
        krhdi->HDIAggrFunc = (KRHDIAggrFunc )kr_hdi_aggr_func;
    } else if (hdi_def->caHdiType[0] == KR_RETRIEVE_SELFDEF) {
        //TODO:get the retrieve data function from so library
        krhdi->HDIAggrFunc = NULL;
    } else {
        KR_LOG(KR_LOGERROR, "HDI Type [%s] error!", hdi_def->caHdiType);
        kr_free(krhdi);
		return NULL;
    }
...
T_KRHDITable *kr_hdi_table_construct(T_KRShmHDI *shm_hdi)
{
    ...
    for (i=0; i<shm_hdi->lHDIDefCnt; ++i) {
        krhdi = kr_hdi_construct(&shm_hdi->stShmHDIDef[i]);
        if (krhdi == NULL) {

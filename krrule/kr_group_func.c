...
void _set_cjson_field(E_KRFieldType type, char *name, U_KRFieldVal *value, cJSON *json) 
{ 
    switch(type) 
    { 
...
void _set_cjson_record(T_KRRecord *krrecord, cJSON *json)
{
    ...
    for (i = 0; i<ptTable->iFieldCnt; i++) {
        E_KRFieldType type = kr_get_field_type(krrecord, i);
...
int kr_group_func(T_KRGroup *krgroup, T_KRContext *krcontext)
{
    ...
    kr_list_foreach(krgroup->ptRuleGroup->ptRuleList, (KRForEachFunc )_add_rule, rules);

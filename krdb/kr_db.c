...
T_KRDB* kr_db_startup(T_DbsEnv *dbsenv, char *dbname, T_KRModule *krdbmodule)
{
    ...
    while(1)
    {
        ...
        /*get field number of this datasrc*/
...
{
    kr_list_foreach(ptKRDB->pTableList, \
			(KRForEachFunc )_kr_db_load_table, NULL);
    return 0;
}
...
T_KRList* kr_db_select(T_KRDB *ptKRDB, int iTableId, int iIndexId, void *key)
{
    ...
    T_KRIndex *ptIndex = kr_get_table_index(ptTable, iIndexId);
    if (ptIndex == NULL) {

...
def on_queue_declared(frame):
    ...
    for x in xrange(0, 10):
        custno = "6223000000000000%04i" % x
        txndate = time.strftime("%Y%m%d", time.localtime())
        txntime = time.strftime("%H%M%S", time.localtime())
        flowno = "%015i" % x
        txntype = "%02i" % (x%2)
        txnamt = x*9.9
        txnloc = "trans location:%02i" % x
        message = "%20s %8s %6s %15s %2s %d %-100s " % (custno, txndate, txntime, flowno, txntype, txnamt, txnloc)
